        ...
        <div class="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($section['items'] as $item)
                @if (! empty($item['heading']) || ! empty($item['text']))
                    <div class="rounded-xl border border-slate-100 p-6 shadow-sm">
                        <div class="mb-3 flex h-10 w-10 items-center justify-center rounded-full text-white" style="background: {{ $primaryColor }}">✓</div>
